...
export default function QuizResults({
    ...
    return (
        <div className="min-h-screen p-4 py-8 overflow-x-hidden">
            <div className="max-w-3xl mx-auto space-y-6">
                <Card className="p-6 md:p-8 text-center space-y-6">
                    ...
                    <div className="space-y-2">
                        <h1 className="text-2xl md:text-4xl font-bold text-balance">{performance.title}</h1>

                        <p className="text-lg text-muted-foreground text-pretty break-words [overflow-wrap:anywhere] hyphens-auto">
                            {performance.message}
                        </p>
                    </div>
                        ...
                        {questions.map((question, index) => {
                            ...
                            return (
                                <div key={question.id} className="pb-6 border-b last:border-b-0 last:pb-0">
                                    {/* ✅ min-w-0 para que el bloque de texto pueda encoger en flex */}
                                    <div className="flex items-start gap-3 mb-4 min-w-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="font-medium text-base md:text-lg leading-relaxed mb-4 text-pretty break-words [overflow-wrap:anywhere] hyphens-auto">
                                                {question.question}
                                            </p>
                                ...
                                <span className="font-semibold text-sm shrink-0">
                                  {optionLabels[optionIndex]}.
                                                                ...
                                                                <span className="text-sm leading-relaxed flex-1 min-w-0 whitespace-normal break-words [overflow-wrap:anywhere] hyphens-auto text-pretty">
                                  {option}
                                </span>
                                                                {isCorrectAnswer && (
                                                                    <span className="ml-2 text-xs font-semibold text-primary shrink-0">
                                    Correct
                                  </span>
                                                                )}
                                                                {isUserAnswer && !isCorrectAnswer && (
                                                                    <span className="ml-2 text-xs font-semibold text-destructive shrink-0">
                                    Your answer
                                  </span>
                                                                )}
                                                            </div>
